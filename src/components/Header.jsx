...
function Header() {
  ...
  <div className="flex flex-wrap items-center justify-end flex-col sm:flex-row">
    ...
    {!isLogged && (
      <>
        <Link to="/login"><button className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 md:py-3 md:px-5 rounded-full ">Login</button></Link>
        <Link to="/signup"><button className="bg-gray-700 hover:bg-gray-800 text-white py-2 px-4 md:py-3 md:px-5 rounded-full mt-2 md:mt-0 md:ml-3">Sign Up</button></Link>
      </>
    )}

    {isLogged && (
      <>
        <button className='bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 md:py-3 md:px-5 rounded-full ml-3' onClick={handleLogout}>Logout</button>
